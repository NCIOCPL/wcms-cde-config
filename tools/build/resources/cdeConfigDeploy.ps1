<#
    .SYNOPSIS
       Deploys CDE configuration files.
    .DESCRIPTION
       Deploys the CDE configuration files from a subdirectory to all the CDE sites
...
function Deploy ($sourceLocation, $env) {
    ...
    foreach( $site in $SITE_LIST ) {
        ...
        foreach( $subsite in $SUBSITE_LIST ) {
            ...
            $config_source = Join-Path -path $sourceLocation "\_configFiles\$deployEnv\$site\$subsite\code\Web.config"

			Write-Host "Copying '$config_source' to '$destination'"
